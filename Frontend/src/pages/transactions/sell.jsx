...
import './buy.css'; // Reusing buy.css for consistent styling
...
const Sell = () => {
  ...
  const handleSell = async (stock) => {
    ...
    if (qty <= 0 || qty > stock.quantity) {
      return alert('Enter a valid quantity to sell (cannot exceed owned quantity)');
    }

    try {
      await axios.post('http://localhost:5001/api/transaction/sell', {
      ...
      setMessage(`Sold ${qty} of ${stock.symbol}`);
      setQuantities({ ...quantities, [stock.symbol]: 0 });
    } catch (err) {
      console.error(err);
      setMessage(`Error selling stock: ${err.response?.data?.error || err.message}`);
    }
  };
  ...
  return (
    <div className="buy-container">
      <div className="buy-header">
        <h1>Sell Stocks</h1>
        <button className="all-stocks-btn" onClick={() => navigate('/buy')}>
          📊 Buy Stocks
        </button>
      </div>
